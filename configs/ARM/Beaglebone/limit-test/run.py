...
parser = argparse.ArgumentParser(description='This is the limit-test run script '
                                 'it demonstrates how a run script could look like '
                                 'and of course starts the limit-test config')
...
try:
    ...
    launcher.start_process("configserver -n limit-test ~/Machineface ")
    if args.video:
        launcher.start_process('videoserver --ini video.ini Webcam1')
    launcher.start_process('linuxcnc limit-test.ini')
